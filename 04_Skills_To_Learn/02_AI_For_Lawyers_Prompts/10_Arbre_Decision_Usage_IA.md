...
Tâche = recherche jurisprudentielle pointue ?
 ├─ OUI → IA non (hallucination) → jep.ma / sgg.gov.ma manuel
 └─ NON → Tâche = draft/structure ?
       ├─ OUI → Donnée sensible ?
       │     ├─ OUI → Local (Ollama/Mistral) + anonymiser
       │     └─ NON → Claude/ChatGPT + garde-fou
       └─ NON → Tâche = conseil final ?
             └─ OUI → Humain only, IA interdite
```

## Checklist 10 sec

- [ ] Anonymisé?
- [ ] Garde-fou "n'invente pas" ajouté?
- [ ] Temps relecture prévu 15 min?
Si 1 non → ne lance pas.
