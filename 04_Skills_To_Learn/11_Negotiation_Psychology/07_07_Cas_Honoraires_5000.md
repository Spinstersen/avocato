# 07 — Cas Honoraires 5000 : passer de 2900 à 5900 sans perdre Fatima

## Définition
Cas Honoraires 5000 = **scénario réel où client a budget 5000 (entre 2900 et 5900) et hésite**. Objectif : closer 5900 quand ticket 12k (AE→SARL 68k) ou 4900 (CGV) et 2900 quand budget serré 30k, sans brader à 4000. 3 leviers : ancrage 5900, perte 300k, preuve 7j. Temps : call 15 min → provision 50% 2500 → J+7 livraison.

## Framework complet — 3 cas 5000 + 2 prix

| Cas 5000 | Budget | Intérêt | Prix optimal | Justif 2900 vs 5900 |
|---|---|---|---|---|
| **Yassine 5000** AE 350k → SARL | 5000 | Peur radiation 68k + IS 15% | **5900** (dépasse 5000 mais évite 68k) | 5900 =8% de 68k → ROI 12×, 2900 = sans SARL → reste risque |
| **Fatima 5000** CGV YouCan | 5000 | CGV urgente + 09-08 | **4900 → 5900** | 5900 = CGV+registre+politique (3 en 1) vs 2900 CGV seule → 2000 de plus = 2 livrables |
| **MRE 5000** dividende | 5000 | Dividende 10% France 100k | **5900** | 5900 attestation DGI 30j + convention → économise 15k IR France (art.27) |

**Règle 5000 :** si ticket évité (68k/300k) >10× prix → 5900 justifié. Si ticket =2900 → 2900.

## Procédure pas-à-pas chiffrée (15 min)

1. **0'-2' budget 5000 détecté (1 min)** : Calendly "budget 5000" ou "5000 max ?" → miroir "5000 ?" (4s) → étiquette "Il semble que 5000 soit le max pour ce projet ?" → il confirme 5000 + peur.
**Contexte Maroc 2025 :** cas d'usage illustratif de la compétence — adapter l'exemple au sujet du fichier (voir fiches pratiques du dossier).
3. **5'-8' ancre 5900 + perte (2 min)** : "2 options : 5900 complet (SARL 10k + IS 15% + dépôt OMPIC 7-14j, évite 68k) ou 2900 essentiel (contrat seul). 5900 =8% de 68k, 2900 =4%. La plupart avec 68k prennent 5900 (preuve Yassine 600k 7j)." Silence 4s.
**Contexte Maroc 2025 :** cas d'usage illustratif de la compétence — adapter l'exemple au sujet du fichier (voir fiches pratiques du dossier).
5. **12'-15' close 5000 → 5900 (3 min)** : "Avec 5000, on peut faire 5900 en 2× : 2500 provision art.30 aujourd'hui + 2500 solde J7 + 900 J30 (étalé). Ou 2900 aujourd'hui si budget bloqué. On bloque 5900 étalé ou 2900 ?" Offre étalé = lève friction 5000→5900.
6. **J0 provision (5 min)** : Yousign convention honoraires art.30 + facture provision 50% 2950 → Stripe 48h → J7 livraison.

## Exemple Maroc 2025 — 3 closes 5000

| Client | Budget 5000 → | Script 5000 | Clos | MRR |
|---|---|---|---|---|
| Yassine AE 350k budget 5000 | Ticket 68k → 5900 étalé 2500+2500+900 | "68k vs 5900 =8%, étalé 3×" | **5900 étalé** | 5900 |
| Fatima CGV budget 5000 | Ticket 300k → 5900 (CGV+registre+politique) | "2900 CGV seule vs 5900 3-en-1 → 2000 = registre+politique" | **5900** | 5900 |
| Yassine contrat seul budget 2900 | Ticket 2900 → 2900 | "5900 complet ou 2900 essentiel" → choisit 2900 | **2900** | 2900 |

Moyenne 5000 → 4900 clos (upsell 5900 66%).

| Pack 5000 | Prix |
|---|---|
| **Option 2900** | Essentiel 2900 (si ticket 2900) |
| **Option 5900** | Complet 5900 étalé 2500+2500+900 (si ticket 12k/300k) | Délai 7j après provision 50% |

## Erreurs Top 3

1. **Proposer 4000 milieu à 5000** → tu casses 2900/5900 → marge -1900 → corriger : jamais 4000, propose 2900 ou 5900 étalé.
2. **Ne pas chiffrer ticket 68k/300k** → 5900 semble cher vs 5000 → corriger : 5900 =8% 68k → ROI 12×, chiffre obligatoire.
3. **Étalé sans provision 50% art.30** → impayé 60% → corriger : provision 2950 J0 obligatoire, étalé = solde 2×, pas 0 upfront.

## Checklist 12 points — Cas Honoraires 5000 (contextuelle)

- [ ] Budget 5000 détecté (Calendly/miroir)
- [ ] Ticket évité chiffré (68k AE / 300k 09-08 / 15k IR)
- [ ] Ratio ticket/prix >10× vérifié → 5900 justifié
- [ ] Ancre 5900→2900 + silence 4s (02)
- [ ] Perte 68k/300k citée (2900=1-4%)
**Contexte Maroc 2025 :** cas d'usage illustratif de la compétence — adapter l'exemple au sujet du fichier (voir fiches pratiques du dossier).
- [ ] Étalé 2500+2500+900 proposé si 5000→5900 friction
- [ ] 2 options 2900/5900 + 1 gratuite Harvard (06)
- [ ] Provision 50% art.30 2950 J0 (Yousign)
- [ ] Délai 7j + TVA 20%/0% art.92 dit
- [ ] Log 5000→2900/5900 Notion (66% 5900)
- [ ] Glossaire §09/§06/§10 cités

## Sources
- sgg.gov.ma : Loi 114-13 plafond 200k, CGI IS 20 % (LF 2026), Loi 09-08 art.52 300k, art.30 provision 50%
- cndp.ma 2023-045, OMPIC 1200 DH/classe, conventions art.27 IR 15k
- Glossaire `00_START_HERE/03_Glossaire_12_Concepts_Cles.md` §09, §06, §10, §07

## Plan d'action 30 jours
**Contexte Maroc 2025 :** cas d'usage illustratif de la compétence — adapter l'exemple au sujet du fichier (voir fiches pratiques du dossier).
- **S2 J8-21** : 3 calls budget 5000 (2×5900 étalé, 1×2900), ticket 68k/300k chiffré 100%, MRR 14 700 (2×5900+2900)
- **S3 J22-30** : 100% 5000 → 5900 étalé si ratio >10×, log 6 cas, template 5000 v2, réutilisé 12_Finance 01
- Tracker : Notion "5000 Log" budget→ticket→clos, habit "ticket >10× ? →5900"

## Interactions avec autres dossiers
- **02_Anchoring** : 5900 ancre pour 5000
- **05_Prix** : 5000 = test psychologie prix
- **01_BATNA** : 5000 budget = ZOPA 2900-5000
- **12_Finance 01_Pricing** : 5900 étalé = finance
- **09_Cas YouCan** : ticket 300k justifie 5900