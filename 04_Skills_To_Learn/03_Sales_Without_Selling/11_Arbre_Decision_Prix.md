...
Prospect budget <2000?
 ├─ OUI → Proposer diagnostic 600 seul + formation groupe
 └─ NON → Risque principal = contrat?
       ├─ OUI → Essentielle 2900
       └─ NON → Risque = 09-08?
             ├─ OUI → Étendue 5900 ou Premium 12k
             └─ NON → Pack sur mesure (devis 24h)
```

## Règle

Toujours 2 options max. 3 = confusion, 1 = pression.
